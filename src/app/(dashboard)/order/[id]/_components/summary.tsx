...
export default function Summary({
  ...
  const { service, tax, totalPrice, grandTotal } = usePricing(orderMenu);
  ...
  const handleGeneratePayment = () => {
    ...
    formData.append("id", order?.customer_name || "");
    startTransition(() => {
      generatePaymentAction(formData);
    });
  };
  useEffect(() => {
    if (genereratePaymentState?.status === "error") {
      toast.error("Generate Payment Failed ", {
        description: genereratePaymentState.errors?._form?.[0],
      });
    }

    if (genereratePaymentState?.status === "success") {
      window.snap.pay(genereratePaymentState.data.payment_token);
    }
  }, [genereratePaymentState]);
